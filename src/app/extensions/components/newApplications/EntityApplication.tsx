...
export const EntityApplication = ({
  ...
  const [showAssociated, setShowAssociated] = useState(false); // Toggle to show or hide associated joint account holders
  ...
  const [entityType, setEntityType] = useState(""); // Set the entity type that the accounts are being opened for
  ...
  const [isJointContactSelected, setIsJointContactSelected] = useState(false);
  ...
  useEffect(() => {
    ...
    fetchCrmObjectProperties([
      ...
      delete newProperties.middlename; // Delete Middle name
    ...
    runServerless({
      ...
      .then((resp) => {
        if (resp.status === "SUCCESS") {
          ...
          const accountObject =
            resp.response.data.CRM.contact.associations.p_account_object_collection__entity_account.items.map(
              (object) => {
          ...
          const accountObjectCount =
            resp.response.data.CRM.contact.associations.p_account_object_collection__entity_account.total;
          setAccounts(accountObject);
          setAccountCount(accountObjectCount);
          setFormProperties({
            ...formProperties, // copy current form properties
            application_type: "Entity", // update property with value
            hs_pipeline: "50936872",
            hs_pipeline_stage: "103949234",
            subject: "New entity account application",
          });
  ...
  useEffect(() => {
    if (selectedContact) {
      ...
      for (const value of Object.values(modifiedObj)) {
        if (value === null) {
          setAssociationError(true);
          setAssociationErrorValidation(
            "This contact's client profile is incomplete. Please fill out all missing properties before continuing."
          );
  ...
  const submitApplication = () => {
    ...
    runServerless({
      name: "newEntityAccountApplication",
      propertiesToSend: ["hs_object_id"],
        ...
        <Tile>
          <Flex direction={"column"} gap={"extra-large"}>
            <StepIndicator
              ...
              stepNames={["Type", "Root", "Additional Account Holder", "Accounts", "Details"]}
            ...
            <Flex direction={"column"} gap={"md"}>
              ...
              <Flex direction={"row"} gap={"md"} justify={"between"}>
                ...
                <Flex direction={"row"} gap={"sm"} justify={"end"}>
                  <Button
                    onClick={() => {
                      if (applicationType === "New Root" && associatedContactCount > 0) {
                        setCurrentStep(currentStep + 2);
                      } else if (applicationType === "New Root" && associatedContactCount === 0) {
          ...
          <Flex direction={"column"} gap={"extra-large"}>
            ...
            <Flex direction={"column"} gap={"md"}>
              <Flex direction={"row"} gap={"flush"} justify={"between"}>
                ...
                <Flex direction={"column"}>
                  <Heading>Are there any additional account holders?</Heading>
                </Flex>
                <Flex direction={"column"}>
                  <Box></Box>
                </Flex>
              ...
              <Flex direction={"row"} gap={"md"} justify={"between"}>
                ...
                <Flex direction={"column"}>
                  <ToggleGroup
                    name="additional-account-holders"
                    label=""
                    options={[
                      { label: "Yes", value: "Yes" },
                      { label: "No", value: "No" },
                    ...
                    toggleType="radioButtonList"
                    onChange={(items) => {
                      if (items! === "Yes" && associatedContactCount > 0) {
                        setShowAssociated(true);
                        setIsJointContactSelected(true);
                      } else if (items! === "Yes" && associatedContactCount === 0) {
                        setError(true);
                      } else {
                        setShowAssociated(false);
                        handleSelectedContact(undefined);
                        setIsJointContactSelected(undefined);
                      }
                    }}
                  />
                  ;
              ...
              <Flex direction={"row"} gap={"md"} justify={"between"}>
                ...
                <Flex direction={"row"} gap={"sm"} justify={"end"}>
                  <Button
                    ...
                    disabled={
                      (selectedContactId && !associationError) || (!isJointContactSelected && !associationError)
                        ? false
                        : true
                    }
                  >
      ...
      return (
        <Tile>
          <Flex direction={"column"} gap={"extra-large"}>
            <StepIndicator
              currentStep={currentStep}
              stepNames={["Additional Account Holder", "Type", "Accounts", "Details"]}
              circleSize={"extra-small"}
            />
            <Flex direction={"column"} gap={"md"}>
              <Flex direction={"row"} gap={"flush"} justify={"between"}>
